...
class Handler(threading.Thread):
 ...
 def run(self):
  ...
  thought = Thought.deserialize(user_id_bytes+timestamp_bytes+thought_sz_bytes+thought_bytes)
  Handler.lock.acquire()
  try:
   user_dir = self.data_dir.joinpath(str(thought.user_id))
   if not user_dir.exists():
    user_dir.mkdir()
   output_file = user_dir.joinpath(thought.timestamp.strftime('%Y-%m-%d_%H-%M-%S') + '.txt')
